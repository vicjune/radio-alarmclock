...
export class LocalStorage {
  ...
  storage = new Storage('./storage/stored.storage');

  radios: any[];
  alarms: any[];

  _defaultRadioId: number;
  _duration: number;
  _increment: number;
  _lastRadio: number;
  ...
  editAlarm(alarm) {
    ...
    for (const _alarm of this.alarms) {
  ...
  editRadio(radio) {
    ...
    for (const _radio of this.radios) {
      if (_radio.id === radio.id) {
        radioExists = true;
        _radio.label = radio.label;
        const urlChange = _radio.url !== radio.url;
  ...
  deleteRadio(id) {
    for (let i = 0; i < this.radios.length; ++i) {
      if (this.radios[i].id === id) {
        const radioDeletedId = this.radios[i].id;
